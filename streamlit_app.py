...
def sanitize_for_pdf(text: str) -> str:
    """Remove characters unsupported by the default FPDF encoding."""
    if text is None:
        return ""
...
def save_plan_to_history(user_profile: dict, plan_markdown: str, model_used: str) -> None:
    """Append the generated plan and user context to a local JSONL history file."""
    history_dir = Path("data")
    history_dir.mkdir(parents=True, exist_ok=True)
    history_path = history_dir / "plan_history.jsonl"
...
def build_plan_pdf(plan_markdown: str, user_profile: dict) -> bytes:
    ...
    name_label = sanitize_for_pdf(user_profile.get("Name") or "NutriGenie User")
...
if submit_button and client:
    ...
    if plan_output:
        ...
        try:
            ...
            st.warning(f"Plan saved locally failed: {exc}")
